...
class TTFParser
{
	...
	function __construct($file)
	{
		...
		if(!$this->f)
			$this->Error(LocalizedString::getString('error.cannot-open-file', 'Can\t open file:').$file);
	}
	...
	function ParseOffsetTable()
	{
		$version = $this->Read(4);
		if($version=='OTTO')
			$this->Error(LocalizedString::getString('error.opentype', 'OpenType fonts based on PostScript outlines are not supported'));
		if($version!="\x00\x01\x00\x00")
			$this->Error(LocalizedString::getString('error.unrecognized-format', 'Unrecognized file format'));
	...
	function ParseHead()
	{
		...
		if($magicNumber!=0x5F0F3CF5)
			$this->Error(LocalizedString::getString('error.incorrect-magic-number', 'Incorrect magic number'));
		$this->Skip(2); // flags
	...
	function ParseCmap()
	{
		...
		if($offset31==0)
			$this->Error(LocalizedString::getString('error.no-unicode-encoding', 'No unicode encoding found'));
		...
		if($format!=4)
			$this->Error(LocalizedString::getString('error.unexpected-format', 'Unexpected subtable format:').$format);
	...
	function ParseName()
	{
		...
		if($this->postScriptName=='')
			$this->Error(LocalizedString::getString('error.postscript', 'PostScript name not found'));
	}
...
//		$this->tables['head']['data'] = substr_replace($this->tables['head']['data'], "\x00\x00\x00\x00", 8, 4);
	...
	{
		if(!isset($this->tables[$tag]))
			$this->Error(LocalizedString::getString('error.table-not-found', 'Table not found:').$tag);
